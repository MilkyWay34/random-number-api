...
final class IntUniformRandomGenerator implements RandomGeneratorInterface
{
    public function generate(GenerationOptions $options): int
    {
        if ($options->getNumberType() !== GenerationOptions::TYPE_INT) {
            throw new \LogicException('Текущий генератор поддерживает только integer-режим.');
        }

        if ($options->getDistribution() !== GenerationOptions::DISTRIBUTION_UNIFORM) {
            throw new \LogicException('Текущий генератор поддерживает только uniform-распределение.');
        }

        if ($options->getPrecision() !== 0) {
            throw new \LogicException('Текущий генератор не поддерживает дробную точность.');
        }

        if ($options->isUnique()) {
            throw new \LogicException('Текущий генератор не поддерживает режим уникальности.');
        }
